...
namespace HoshinoLabs.IwaSync3.Udon
{
    ...
    public class VideoController : VideoCoreEventListener
    {
        ...
        public void ValidateView()
        {
            ...
            _volumeSlider.SetValueWithoutNotify(core.volume);
            ...
            _minVolumeSlider.SetValueWithoutNotify(core.minVolume);
            _maxVolumeSlider.SetValueWithoutNotify(core.maxVolume);
            ...
            _brightnessSlider.SetValueWithoutNotify(core.brightness);
        }
        ...
        private void Update()
        {
            ...
            else
            {
                ...
                if (core.duration != 0f)
                {
                    ...
                    var time = core.time;
                    _messageTextText.text = core.Message;
                    var progressDrag = _progressDrag || Mathf.Approximately(time, _prevTime);
                    var timeText = TimeSpan.FromSeconds(progressDrag ? (duration * _progressSlider.value) : time).ToString(format);
                    var durationText = TimeSpan.FromSeconds(duration).ToString(format);
                    _messageTimeText.text = $"{timeText}/{durationText}";
                    if (!progressDrag)
                    {
                        _progressSlider.SetValueWithoutNotify(Mathf.Clamp(time / duration, 0f, 1f));
                    }
                }
            }
        }
        ...
        {
            Debug.Log($"[<color=#47F1FF>{IwaSync3.APP_NAME}</color>] Progress changed.");
            _prevTime = core.time;
            core.TakeOwnership();
        ...
        {
            Debug.Log($"[<color=#47F1FF>{IwaSync3.APP_NAME}</color>] Volume changed.");
        ...
        public void SpeedLL()
        {
            ...
            core.clockTime = Networking.GetServerTimeInMilliseconds();
            core.Seek(0f);
            core.speed = Mathf.Max(0.25f, core.speed - 1f);
            core.RequestSerialization();
            ValidateView();
        }
        ...
        public void SpeedL()
        {
            ...
            core.clockTime = Networking.GetServerTimeInMilliseconds();
            core.Seek(0f);
            core.speed = Mathf.Max(0.25f, core.speed - 0.25f);
            core.RequestSerialization();
            ValidateView();
        }
        ...
        public void OnBrightnessChanged() {
            Debug.Log($"[<color=#47F1FF>{IwaSync3.APP_NAME}</color>] Brightness changed.");
            core.brightness = _brightnessSlider.value;
        }
    }
}
