...
public class FpsViewer2_ForClient : UdonSharpBehaviour
{
    ...
    private bool AmIMaster = false;                                                 // 自分がインスタンスマスターだったらtrue
    private bool AlreadyViewOff = false;                                            // すでに表示を非表示にしたかフラグ
    ...
    public void OwnerJdgeCycle()
    {
        ...
        AmIMaster = MyInfo.isMaster;                                                // インスタンスマスターか確認

        // このオブジェクトのオーナーがマスターだったら自分側で非表示にする
        if (false == AlreadyViewOff && true == Networking.GetOwner(this.gameObject).isMaster)
        {
            ViewOff();                                                              // 表示を非表示にする
            AlreadyViewOff = true;                                                  // すでに非表示にしたフラグを立てる
        }
    ...
    void Update()
    {
        // SaveMyInfo()が完了 && このオブジェクトのオーナー && インスタンスマスターじゃない なら実行
        // AmIOwnerとAmIMasterは、OwnerJdgeCycle()内でSyncCycle[s]間隔で随時更新される
        if (true == UpdateOK && true == AmIOwner && false == AmIMaster)
        {
    ...
    public void ViewUpdate()
    {
        ...
        AlreadyViewOff = false;                                                         // すでに非表示にしたフラグを折る
    }
